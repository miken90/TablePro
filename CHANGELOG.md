...
- Toggle Filter now has a real keyboard shortcut (**Ctrl+Alt+F**) and a command palette entry.

- Every Settings control has an accessible name — roughly 19 rows across the eight panes (Toggle, Select, NumberInput, TextInput, PasswordInput, Slider) previously had none. The Appearance theme picker is now a real `role="radiogroup"` of three `role="radio"` cards (Light/Dark/System), arrow-key navigable, instead of a plain button row.
...
- The PostgreSQL table list matched relations by name before narrowing by schema, so every table was briefly paired with every same-named relation elsewhere in the database — other schemas' tables, and indexes and sequences sharing the name. The schema is now resolved to an oid first and the relation matched on schema and name together, which is the catalog's own unique key. The list itself is unchanged; the lookup is narrower and uses the index.
...
- The sidebar populates noticeably faster after connecting. Tables, routines, and the schema list used to load one after another — three of the four round trips only started after the previous one's result triggered a re-render — even though none of them actually depends on another's result. They now fire concurrently; a routines fetch failing (e.g. a permission-restricted database) no longer prevents tables from loading, and vice versa.

### Changed

- Preview SQL and the execute confirmation now show exactly the statements the app will run, generated by the same backend code that executes them. The preview was a separate frontend approximation: it quoted identifiers ANSI-style on every engine, decided a value's type from its shape rather than the column's, and always printed `BEGIN`/`COMMIT` even for a single statement.

- Explain plans now open as a tab in the results strip instead of a band above the grid.

- Row editing now obeys Safe Mode. The grid save called the backend directly, so a Read-Only (level 5) session could still write through the data grid. A refused save keeps the staged edits.

- The quick switcher and command palette are now one overlay: type to search objects, or start with `>` to run a command. Both shortcuts still work.
...
- Grid column headers now print the column type under the column name, on its own line, so a narrow column truncates the name without losing the type.

- Focus outlines were suppressed (`outline-none`, or a `.focus-ring` utility that itself started with `focus-visible:outline-none`) across the grid, connection/history/layout panels, dialogs, and the command kit, with nothing drawn in their place — keyboard focus was invisible in those controls. Outlines are restored everywhere, drawn from the same focus-ring tokens. Secondary/muted text color that was washed out to the point of being hard to read is now full-contrast except where it's genuinely disabled or decorative, and every hover background now pairs with a readable text color instead of occasionally leaving default-weight text on a changed background.

- The sidebar's search and refresh controls now have accessible names (the refresh button previously had only a `title`), pulled into their own header component above the tree.

- The connection card matches the design spec: border and shadow now change together on hover instead of just the fill, and its right-click menu uses the same Menu/MenuItem kit as the rest of the app instead of a hand-rolled list. The connection list's search and "New Connection" controls moved into a header row — search on the left, a split button (primary action + Import/New Group dropdown) on the right — replacing a full-width search block stacked above a centered button row.

- Import SQL, the bulk insert/update/delete dialogs, Create Table, procedure execute/view-source, connection export/import, Safe Mode's confirmation, the destructive table-operation confirmation (Truncate/Delete All/Drop), Settings, and the keyboard-shortcuts help now share one dialog shell: banded header/body/footer, no ad-hoc border, focus trapped, `Esc` closes, focus returns to whatever opened it, and a destructive dialog focuses Cancel first instead of the action that commits the write.
...
- The execute, refresh, discard, unsaved-changes and apply-structure confirmations now open with focus on Cancel. Enter used to land on the button that committed the write.
...
- Settings → General's "NULL display text" now reaches the grid; NULL cells rendered the literal `NULL` regardless of the setting. A blank setting falls back to the default `NULL` rather than an empty cell, which would be indistinguishable from an empty-string value.
...
- The filter tooltip advertised a shortcut (**Ctrl+Shift+L**) that actually belonged to the AI chat toggle. Filter had no keyboard shortcut of its own until now, and the row-action bar's own filter button duplicated the status bar's — a filter applied from one could go unreflected in the other. There is now exactly one filter panel per workspace, toggled only from the status bar.

- Import SQL can no longer be dismissed mid-write (no backdrop click, no `Esc`) and now stays open when an import fails instead of closing and losing the partial-write state. The failure view reports the driver's error verbatim, how many of the file's statements ran before the failure (or says plainly that no progress was reported, never a guessed "0 executed"), and whether the transaction was rolled back or — since the backend discards the rollback's own result — only that a rollback was *requested*. Retry re-runs the whole file (there is no resume point); if the run wasn't wrapped in a transaction and some statements already committed, Retry is relabelled "Re-run entire file" with a warning naming how many will run again.
...
- Cancel now stops the query it is supposed to stop. The owning tab and its session are captured when the query starts, so pressing Stop (or `Escape`) after switching tabs cancels the running statement instead of aborting whatever the newly-active tab's connection happened to be executing. The cancel is also a single round-trip, and a driver that cannot cancel (SQL Server, MongoDB, Redis) no longer leaves the toolbar stuck in the running state.
- Closing a tab now cancels its in-flight query instead of leaving it running in the background and rendering its rows under whichever tab is active afterwards. The same applies to Close Others / Close All / Close to the Right.
- Sidebar **Drop**, **Truncate**, and **Delete All Records** now obey Safe Mode. They previously ran the statement directly, so a Level 5 (read-only) setting still dropped the table. They also build the statement through the backend generator, which fixes MariaDB connections receiving double-quoted identifiers (error 1064) and escapes quote characters inside table and schema names.
- Truncating or deleting all records now requires typing the table name, as dropping already did, and `Enter` cannot confirm any of the three until the name matches.
...
## Windows app changelog (before the repository was flattened)

Until the Windows app moved from `tablepro-windows/` to the repository root, it
kept its own changelog alongside the fork-level one above. Both are preserved
verbatim: the two overlap for shared version numbers, describe some releases at
different levels of detail, and occasionally place the same item under a
different version, so they are not interleaved into one list.

Versions recorded only here: 0.5.0, 0.3.3, 0.3.2, 0.3.1. Recorded only above: 0.6.0.
