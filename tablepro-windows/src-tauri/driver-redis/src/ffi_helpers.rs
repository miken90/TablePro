...
/// Build an FfiQueryResult from column metadata and row data.
///
/// `columns` — (name, type_name, nullable, is_pk)
/// `rows` — row-major cells (Option<String>)
...
/// Build a single-row message result (e.g. "OK", status messages).
pub fn message_result(msg: &str) -> FfiQueryResult {
    let columns = vec![("Result".to_string(), "string".to_string(), false, false)];
    let rows = vec![vec![Some(msg.to_string())]];
