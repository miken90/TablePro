...
pub extern "C" fn tablepro_plugin_metadata() -> PluginMetadata {
    PluginMetadata {
        ...
        type_id: string_to_ffi("redis".to_string()),
        display_name: string_to_ffi("Redis".to_string()),
        default_port: 6379,
    }
}
...
unsafe extern "C" fn create_driver(config: *const DriverConfig) -> *mut DriverHandle {
    ...
    match RedisDriver::new(host, port, user, password, database, ssl_mode) {
...
unsafe extern "C" fn destroy_driver(handle: *mut DriverHandle) {
    if !handle.is_null() {
        drop(Box::from_raw(handle as *mut RedisDriver));
    }
}
...
unsafe extern "C" fn fetch_tables(
    ...
    ops_schema::fetch_tables(handle)
}
