      ...
      <div className="fixed left-1/2 top-[15vh] w-[560px] max-w-[90vw] -translate-x-1/2 animate-slide-down overflow-hidden rounded-lg border border-[var(--color-border)] bg-[var(--color-bg-elevated)] shadow-modal">
        ...
        <div className="flex items-center border-b border-[var(--color-border-subtle)] px-4">
          ...
          <Command.Input
            ...
            className="h-12 flex-1 bg-transparent text-sm text-[var(--color-text-primary)] outline-none placeholder:text-[var(--color-text-muted)]"
          />
          <kbd
            className="shrink-0 rounded border border-[var(--color-border)] bg-[var(--color-bg-muted)] px-1.5 py-0.5 font-mono text-[10px] text-[var(--color-text-muted)]"
          >
            Esc
          </kbd>
        </div>
        ...
        <Command.List className="max-h-[400px] overflow-y-auto p-2">
          ...
          {query === '' && recentCmds.length > 0 && (
            <Command.Group
              heading="Recent"
              className="[&_[cmdk-group-heading]]:mb-1 [&_[cmdk-group-heading]]:px-3 [&_[cmdk-group-heading]]:py-1 [&_[cmdk-group-heading]]:text-[10px] [&_[cmdk-group-heading]]:font-semibold [&_[cmdk-group-heading]]:uppercase [&_[cmdk-group-heading]]:tracking-wider [&_[cmdk-group-heading]]:text-[var(--color-text-muted)]"
            >
              {recentCmds.map((cmd) => (
          ...
          {CATEGORY_ORDER.map((cat) => {
            ...
            return (
              <Command.Group
                key={cat}
                heading={cat}
                className="[&_[cmdk-group-heading]]:mb-1 [&_[cmdk-group-heading]]:mt-2 [&_[cmdk-group-heading]]:px-3 [&_[cmdk-group-heading]]:py-1 [&_[cmdk-group-heading]]:text-[10px] [&_[cmdk-group-heading]]:font-semibold [&_[cmdk-group-heading]]:uppercase [&_[cmdk-group-heading]]:tracking-wider [&_[cmdk-group-heading]]:text-[var(--color-text-muted)]"
              >
                {cmds.map((cmd) => (
                  <CommandItem key={cmd.id} command={cmd} onSelect={() => handleSelect(cmd.id)} />
                ))}
