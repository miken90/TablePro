...
export function RightDock({ isConnected }: RightDockProps) {
  ...
  // Esc closes the dock — the topmost non-modal layer (§6.3: popover →
  // palette → dialog → dock). A higher layer that is actually open has
  // already claimed the event via its own focus-trapped Esc handler, which
  // calls `stopPropagation` before this ever sees it, so no explicit "is a
  // higher layer open" check is needed here. [RT-9]
  useEffect(() => {
    if (!dockOpen) return;
