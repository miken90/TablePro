...
Khi cả quick search và các hàng filter đều đang hoạt động, các điều kiện WHERE của chúng được nối bằng AND — kết quả phải khớp với chuỗi tìm kiếm *và* tất cả các filter đã áp dụng.
...
## Điều Hướng FK qua Filter

Click vào mũi tên khóa ngoại trong data grid sẽ mở bảng được tham chiếu với filter được áp dụng sẵn khớp với giá trị FK. Điều này cho phép bạn nhanh chóng điều hướng các mối quan hệ — ví dụ, click vào mũi tên FK `order.user_id` sẽ mở bảng `users` được lọc theo người dùng cụ thể đó.
