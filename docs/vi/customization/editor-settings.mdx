...
Bật phím tắt Vim cho trình soạn thảo SQL:

| Tùy chọn | Mô tả |
|--------|-------------|
| **Off** | Chỉnh sửa văn bản macOS tiêu chuẩn (mặc định) |
| **On** | Phím tắt kiểu Vim với chế độ normal/insert/visual |

Khi được bật, trình soạn thảo hỗ trợ Vim motions (`h`, `j`, `k`, `l`, `w`, `b`, `e`), operators (`d`, `c`, `y`), chọn vùng visual mode và chế độ command-line (lệnh `:`).

<Tip>
Các phím tắt tiêu chuẩn như `Cmd+Enter` để thực thi truy vấn vẫn hoạt động trong chế độ Vim. Xem [Chế độ Vim](/vi/features/sql-editor#vim-mode) để biết danh sách đầy đủ các lệnh Vim được hỗ trợ.
</Tip>
