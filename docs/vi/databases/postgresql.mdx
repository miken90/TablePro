...
Bạn có thể chuyển đổi giữa các database PostgreSQL bằng trình chuyển đổi database (**⌘K**):

1. Nhấn **⌘K** để mở trình chuyển đổi — hiển thị danh sách database cho kết nối PostgreSQL
2. Chọn database đích (ví dụ: `app`, `analytics`, hoặc `postgres`)
3. Sidebar, truy vấn và toolbar sẽ cập nhật sau khi kết nối lại

<Tip>
Các database hệ thống (`postgres`, `template0`, `template1`) được đánh dấu là hệ thống.
</Tip>

<Note>
Chuyển đổi database sẽ kết nối lại phiên PostgreSQL tới database đã chọn.
</Note>
