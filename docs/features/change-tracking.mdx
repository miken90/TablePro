...
Click **Commit** or press `Ctrl+S`. TablePro generates parameterized SQL, executes it, clears the queue, and refreshes the grid.
...
Click the **Preview SQL** button (eye icon) or press `Ctrl+Shift+P` to see the exact SQL before committing. Use **Copy All** to copy to clipboard.
