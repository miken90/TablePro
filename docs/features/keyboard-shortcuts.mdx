...
<Warning>
**Platform Note**: This documentation uses macOS shortcuts (`Cmd`, `Option`). On **Windows**, substitute `Ctrl` for `Cmd`, `Alt` for `Option`, and `Shift` works the same way. For example, `Cmd+Enter` becomes `Ctrl+Enter`, and `Option+Cmd+F` becomes `Alt+Ctrl+F`.
</Warning>
...
|--------|----------|
| Query History | `Cmd+Y` |
| Toggle Cell Inspector | `Cmd+Shift+I` |
...
| **View** | Toggle Sidebar, Toggle Cell Inspector, Toggle Filter, Query History |
