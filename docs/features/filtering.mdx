---
title: Filtering
description: Filter and search your data with visual tools
---
...
When both quick search and filter rows are active, their WHERE conditions are joined with AND — the results must match the search text *and* all applied filters.
...
## FK Navigation Filter

Clicking a foreign key arrow in the data grid opens the referenced table with a pre-applied filter matching the FK value. This lets you quickly navigate relationships — for example, clicking an `order.user_id` FK arrow opens the `users` table filtered to that specific user.
