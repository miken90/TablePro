---
title: Data Grid
description: View, edit, and manage your data in a spreadsheet-like interface
---
...
- Column widths are remembered for each table and persist across pagination, sorting, and filtering reloads within the tab session
...
Hold `Shift` and click additional column headers to add them to the sort list. Each sorted column shows a priority indicator in its header (e.g., 1▲, 2▼). Clicking a column header without `Shift` replaces the entire sort with that single column.
...
Foreign key columns display a navigation arrow icon in each cell. Click the arrow to open the referenced table, automatically filtered to the row matching the FK value. This lets you quickly jump between related tables without writing queries.
...
| With headers | `Cmd+Shift+C` | Tab-separated with column headers as the first line |
| JSON | `Cmd+Option+C` | JSON array of objects |
| CSV | Available from context menu | Comma-separated values |

"Copy with Headers" (`Cmd+Shift+C`) prepends column names as the first TSV line, which is useful when pasting into spreadsheets or sharing data snippets. Also available via the right-click context menu.
...
<Tip>
Quick search and filter rows can be used together — their WHERE conditions are joined with AND, so results must match both criteria. See [Filtering](/features/filtering) for details.
</Tip>
