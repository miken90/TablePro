...
- **Preview SQL**: Click the **Preview SQL** button (eye icon) or press `Ctrl+Shift+P` to review pending SQL statements before committing
...
The Cell Inspector is a right sidebar panel showing detailed information about the selected row or current table. Toggle with `Ctrl+Shift+B` or via **View** > **Toggle Cell Inspector**.
...
Select cells and press `Ctrl+C` for TSV, `Ctrl+Shift+C` for CSV, or `Ctrl+Alt+J` for JSON.
