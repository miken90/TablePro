...
|---------|---------|---------|
| **CodeEditSourceEditor** | `main` branch | Tree-sitter-powered code editor component for the SQL editor |
...
Monitors active database connections with a 30-second ping interval. Automatically reconnects on failure using exponential backoff. Runs per-session and cleans up when the session disconnects.

### MainContentCoordinator

Central coordinator for the main content area, split across 14 extension files in `Views/Main/Extensions/`:
...
When adding coordinator functionality, add a new extension file rather than growing the main file.
...
| Tab state | JSON files (App Support) | `TabPersistenceService` / `TabStateStorage` |
| Filter presets | JSON | `FilterSettingsStorage` |
| Connection groups | JSON | `GroupStorage` |
| Connection tags | JSON | `TagStorage` |
