...
TablePro uses trust-on-first-use (TOFU) host-key verification. On first connection to a server, the fingerprint is stored automatically at `%APPDATA%\TablePro\known_hosts.json` — there is no prompt to review or approve it.

If a previously trusted server's host key changes, the connection is rejected outright (logged as an error). This could mean the server was reinstalled, or it could indicate a security issue. There is no accept/abort dialog — remove the stale entry from `known_hosts.json` to trust the new key.
...
TablePro does not send its own keep-alive packets on the tunnel. If it drops:
1. Check network stability
2. Verify server's `ClientAliveInterval` setting
3. Check for idle timeout settings on firewalls
4. Reconnect manually — see [Reconnecting](/databases/overview#reconnecting)
