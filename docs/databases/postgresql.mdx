...
You can switch between PostgreSQL databases using the database switcher (**⌘K**):

1. Press **⌘K** to open the switcher — it lists databases for PostgreSQL connections
2. Select the target database (for example, `app`, `analytics`, or `postgres`)
3. The sidebar, queries, and toolbar update after the connection reconnects

<Tip>
System databases (`postgres`, `template0`, `template1`) are marked as system entries.
</Tip>

<Note>
Database switching reconnects your PostgreSQL session to the selected database.
</Note>
